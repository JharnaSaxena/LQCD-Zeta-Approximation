...
# Boosts with representative gamma
BOOSTS = {
...
# PLOT FUNCTION (clean, professional)
def plot_comparison(boost_key, d, gamma, name, poly_data, pade3_data, pade4_data, pade5_data, pade6_data, spline_data):
    ...
    plt.close()
    print(f" lot saved: {outfile}")
    
    return {
        'poly': np.mean(error_poly[valid]) if valid.any() else np.nan,
        'pade3': np.mean(error_pade3[valid]) if valid.any() else np.nan,
        'pade4': np.mean(error_pade4[valid]) if valid.any() else np.nan,
        'pade5': np.mean(error_pade5[valid]) if valid.any() else np.nan,
        'pade6': np.mean(error_pade6[valid]) if valid.any() else np.nan,
        'spline': np.mean(error_spline[valid]) if valid.any() else np.nan,
    }
...
def main():
    ...
    print("METHOD ACCURACY COMPARISON - Mean Absolute Errors")
    # Header
    print(f"{'Boost':<8}", end="")
    for method in ['Poly deg6', 'Padé3', 'Padé4', 'Padé5', 'Padé6', 'Spline']:
        print(f"{method:>12}", end="")
    print()
    for boost_key, err in results.items():
        print(f"{boost_key:<8}", end="")
        print(f"{err['poly']:12.2e}" if not np.isnan(err['poly']) else f"{'FAILED':>12}", end="")
        print(f"{err['pade3']:12.2e}" if not np.isnan(err['pade3']) else f"{'FAILED':>12}", end="")
        print(f"{err['pade4']:12.2e}" if not np.isnan(err['pade4']) else f"{'FAILED':>12}", end="")
        print(f"{err['pade5']:12.2e}" if not np.isnan(err['pade5']) else f"{'FAILED':>12}", end="")
        print(f"{err['pade6']:12.2e}" if not np.isnan(err['pade6']) else f"{'FAILED':>12}", end="")
        print(f"{err['spline']:12.2e}" if not np.isnan(err['spline']) else f"{'FAILED':>12}", end="")
        print()
    
    print("=" * 100)
    print("\nBEST METHOD PER BOOST:")
    for boost_key, err in results.items():
        errors = {k: v for k, v in err.items() if not np.isnan(v)}
        if errors:
            best = min(errors, key=errors.get)
            method_names = {'poly': 'Polynomial deg6', 'pade3': 'Padé [3/3]', 'pade4': 'Padé [4/4]',
                           'pade5': 'Padé [5/5]', 'pade6': 'Padé [6/6]', 'spline': 'Spline'}
            print(f"   {boost_key}: {method_names[best]} (error: {errors[best]:.2e})")
